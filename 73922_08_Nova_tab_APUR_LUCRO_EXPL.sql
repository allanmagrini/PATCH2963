  ...
  begin
   ...
   begin
      select count(1) 
       ...
       where upper(trim(s.sequence_name)) = 'APURLUCROEXPL_SEQ'
         and upper(trim(s.table_name))    = 'APUR_LUCRO_EXPL';	  
   exception
      when others then
         vn_qtde := 0;
   end;
   ...
   if vn_qtde = 0 then
      ...
      exception
         when dup_val_on_index then
            null;	  
         when others then
            RAISE_APPLICATION_ERROR ( -20101, 'Erro ao inserir sequence APURLUCROEXPL_SEQ na tabela SEQ_TAB  - '||SQLERRM );
      end;		
      --		 
   end if;	  
   --     
