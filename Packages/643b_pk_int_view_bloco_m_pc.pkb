...
begin
  ...
  if vn_infadicdifpis_id is null then
    ...
    vn_fase := 2.1;
    --
    select infadicdifpis_seq.nextval 
      into vn_infadicdifpis_id
      from dual;
    --
    ...
    insert into inf_adic_dif_pis
                (id,
                 ...
                 vl_cred_dif,
                 tipocredpc_id)
       values (gt_row_inf_adic_dif_pis.id,
    ...
    pkb_seta_referencia_id(en_id => gt_row_inf_adic_dif_pis.id);
    ----
    update inf_adic_dif_pis
       set vl_vend      = gt_row_inf_adic_dif_pis.vl_vend,
...
exception
  when others then
    --
    begin
      pk_agend_integr.gvtn_qtd_erro(gv_cd_obj) := nvl(pk_agend_integr.gvtn_qtd_erro(gv_cd_obj),
                                                      0) + 1;
    exception
      when others then
        null;
    ...
    --
    gv_resumo := 'Erro na pk_int_view_bloco_m_pc.prc_integr_inf_adic_dif_pis fase (' ||
                 vn_fase || '): ' || sqlerrm;
    --
    declare
      vn_loggenerico_id log_generico_ibmpc.id%TYPE;
...
begin
  ...
  if vn_infadicdifcofins_id is null then
    ...
    vn_fase := 2.1;
    --
    select infadicdifcofins_seq.nextval
      into vn_infadicdifcofins_id
      from dual;
    --  
    ...
    insert into inf_adic_dif_cofins
                (id,
                 ...
                 vl_cred_dif,
                 tipocredpc_id)
         values (gt_row_inf_adic_dif_cofins.id,
    ...
    ----
    vn_fase := 2.3;
    update inf_adic_dif_cofins
       set vl_vend      = gt_row_inf_adic_dif_cofins.vl_vend,
...
exception
  when others then
    ...
    --
    gv_resumo := 'Erro na pk_int_view_bloco_m_pc.prc_integr_inf_adic_dif_cofins fase (' ||
                 vn_fase || '): ' || sqlerrm;
    --
    declare
      vn_loggenerico_id log_generico_ibmpc.id%TYPE;
...
procedure pkb_inf_adic_dif_pc
  is
   ...
   --
   --#76133
   vt_log_generico_ibmpc    dbms_sql.number_table;
   --
   x                        pls_integer;  -- P/Pis
...
begin
   ...
   --
   -- #76133 - inclusao do calculo erp 
   -- Calcula a quantidade de registros buscados no ERP
   -- para ser mostrado na tela de agendamento.
   --
   begin
     pk_agend_integr.gvtn_qtd_erp(gv_cd_obj) := nvl(pk_agend_integr.gvtn_qtd_erp(gv_cd_obj),0) + nvl(vt_tab_csf_inf_adic_dif_pc.count,0);
   exception
     when others then
      null;
   end;
      ...
      for i in vt_tab_csf_inf_adic_dif_pc.first..vt_tab_csf_inf_adic_dif_pc.last loop
         ...
         if vt_tab_csf_inf_adic_dif_pc(i).dm_ind_pc = '4' then
           ...
           if vn_detconscontrpis_id <> 0 then
              --
              vn_ccp_id     := null ;--#76133
              vn_ccp_dm_sit := null ;--#76133
              --
              vn_fase := 7.4;
              --
              ...
              if vn_ccp_dm_sit in ('1','2','3','4') then
                 ...
                 --
                 if vn_ccp_id is not null then 
                   --
                   pk_apur_pis.pkb_desprocessar_cons_pis_m200(en_conscontrpis_id => vn_ccp_id);
                   --
                 else
                    --
                    gv_resumo := ' Nao foi localizado o id e/ou status da tabela CONS_CONTR_PIS(M200): Id = '||vn_ccp_id
                                  || ' / status = ' ||vn_ccp_dm_sit 
                                  ||'. Verifique se existe o registro na tabela DET_CONS_CONTR_PIS com o id = '||vn_detconscontrpis_id||'.' ;
                    ----
                    declare
                       vn_loggenerico_id  log_generico_ibmpc.id%TYPE;
                    begin
                       ...
                       pkb_log_generico_ibmpc ( sn_loggenericoibmpc_id  => vn_loggenerico_id
                                              , ev_mensagem             => gv_mensagem_log
                                              , ev_resumo               => gv_resumo
                                              , en_tipo_log             => ERRO_DE_SISTEMA--informacao --#76133
                                              , en_referencia_id        => gn_referencia_id
                                              , ev_obj_referencia       => gv_obj_referencia
                                              , en_empresa_id           => gn_empresa_id
                                              , en_dm_impressa          => 0
         ...
         if vt_tab_csf_inf_adic_dif_pc(i).dm_ind_pc = '5' then
           ...
           if vn_detconscontrcofins_id <> 0 then
              --
              vn_ccc_id     := null ;--#76133
              vn_ccc_dm_sit := null ;--#76133
              --
              vn_fase := 7.7;
              --
              ...
              if vn_ccc_dm_sit in ('1','2','3','4') then
                 ...
                 --
                 if vn_ccc_id is not null then 
                   --
                   pk_apur_cofins.pkb_desproc_cons_cofins_m600(en_conscontrcofins_id => vn_ccc_id);
                   --
                 else
                    --
                    gv_resumo := ' Nao foi localizado o id e/ou status da tabela CONS_CONTR_COFINS(M600): Id = '||vn_ccc_id
                                  || ' / status = ' ||vn_ccc_dm_sit 
                                  ||'. Verifique se existe o registro na tabela DET_CONS_CONTR_COFINS com o id = '||vn_detconscontrcofins_id||'.' ;
                    ----
                    declare
                       vn_loggenerico_id  log_generico_ibmpc.id%TYPE;
                    begin
         ...
         begin
            ...
            if pk_agend_integr.gvtn_qtd_total(gv_cd_obj) >
             ...
             --
             if nvl(vt_log_generico_ibmpc.count,0) > 0 then -- Erro de validação
              --
              pk_agend_integr.gvtn_qtd_erro(gv_cd_obj) := nvl(pk_agend_integr.gvtn_qtd_erro(gv_cd_obj),0) + 1;
              --
             else
...
begin
  ...
  if vn_contrpisdpant is null then
    ---
    select contrpisdifperant_seq.nextval
      into vn_contrpisdpant 
      from dual;
    --
    ...
    insert into contr_pis_dif_per_ant
                ...
                per_apur ,
                dt_receb) 
       values ( gt_row_contr_pis_dif_per_ant.id ,
    ...
    gt_row_contr_pis_dif_per_ant.id := vn_contrpisdpant;
    ---
    update contr_pis_dif_per_ant
       set vl_cont_apur_difer = gt_row_contr_pis_dif_per_ant.vl_cont_apur_difer,
...
begin
   ...
   --
   begin
     pk_agend_integr.gvtn_qtd_erp(gv_cd_obj) := nvl(pk_agend_integr.gvtn_qtd_erp(gv_cd_obj),0) + nvl(vt_tab_csf_contr_pis_dif_pant.count,0);
   exception
     when others then
      null;
   end;
...
begin
  ...
  if vn_contrcofdpant is null then
    ---
    select contrcofinsdifperant_seq.nextval
      into vn_contrcofdpant
      from dual;
    --
    ...
    insert into contr_cofins_dif_per_ant
            (id,
             ...
             per_apur,
             dt_receb)
    values  (gt_row_contr_cof_dif_per_ant.id,
    ...
    gt_row_contr_cof_dif_per_ant.id := vn_contrcofdpant;
    ---
    update contr_cofins_dif_per_ant
       set vl_cont_apur_difer = gt_row_contr_cof_dif_per_ant.vl_cont_apur_difer,
...
begin
   ...
   --
   begin
     pk_agend_integr.gvtn_qtd_erp(gv_cd_obj) := nvl(pk_agend_integr.gvtn_qtd_erp(gv_cd_obj),0) + nvl(vt_tab_csf_contr_cof_dif_pant.count,0);
   exception
     when others then
      null;
   end;
