...
begin
   ...
   for x in (
      select ora.id obrigrecapuricms_id
           , ai.id apuracaoicms_id
           , pdgi.empresa_id_guia
           , pdgi.obs
           , e.pessoa_id
           , ai.dt_inicio
           , last_day(ai.dt_fim) dt_ref
           , pdgi.pessoa_id_sefaz
           , pdgi.dm_origem
           , pdgi.dm_tipo
           , pdgi.planoconta_id
           , pdgi.dia_vcto
           , add_months(ai.dt_fim,1)  dt_vcto
           , cur.cod_rec              cod_receita
      ...
      where ai.id                 = ora.apuracaoicms_id
        ...
        and pdgi.dm_origem        = 3                             -- Apuração ICMS
        and e.id                  = pdgi.empresa_id_guia
        and cur.id            (+) = ora.codrecuf_id
   ...
   loop
      ...
      if nvl(x.vl_orig_rec,0) > 0 then
         ...
         pk_csf_api_gpi.gt_row_guia_pgto_imp.dm_ret_erp               := case pk_csf.fkg_parametro_geral_sistema(pk_csf.fkg_multorg_id_empresa(x.empresa_id_guia), x.empresa_id_guia, 'GUIA_PGTO', 'RET_ERP', 'LIBERA_AUTOM_GUIA_ERP') when '1' then 0 when '0' then 6 end;
