...
begin
  begin
   ...
   begin
      select count(1) 
        into vn_qtde		 
        from csf_own.seq_tab s
       where upper(trim(s.sequence_name)) = 'PARAMDPCODDINCTALUCREXPL_SEQ'
         and upper(trim(s.table_name))    = 'PARAM_DP_CODDIN_CTA_LUCR_EXPL';	  
   exception
      when others then
         vn_qtde := 0;
   end;
      ...
      begin
         insert into csf_own.seq_tab ( id
                             ...
                             values ( csf_own.seqtab_seq.nextval
                                    , 'PARAMDPCODDINCTALUCREXPL_SEQ'
                                    , 'PARAM_DP_CODDIN_CTA_LUCR_EXPL');
      exception
         when dup_val_on_index then
            null;	  
         when others then
            RAISE_APPLICATION_ERROR ( -20101, 'Erro ao inserir sequence PARAMDPCODDINCTALUCREXPL_SEQ na tabela SEQ_TAB  - '||SQLERRM );
      end;		
      --		 
   end if;	  
   --     
