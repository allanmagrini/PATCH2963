  ...
  begin
   ...
   begin
      select count(1) 
       ...
       where upper(trim(s.sequence_name)) = 'CRITPESQSLDLUCREXPL_SEQ'
         and upper(trim(s.table_name))    = 'CRIT_PESQ_SLD_LUCR_EXPL';	  
   exception
      when others then
         vn_qtde := 0;
   end;
      ...
      begin
         insert into csf_own.seq_tab ( id
                             ...
                             values ( csf_own.seqtab_seq.nextval
                                    , 'CRITPESQSLDLUCREXPL_SEQ'
                                    , 'CRIT_PESQ_SLD_LUCR_EXPL');
      exception
         when dup_val_on_index then
            null;	  
         when others then
            RAISE_APPLICATION_ERROR ( -20101, 'Erro ao inserir sequence CRITPESQSLDLUCREXPL_SEQ na tabela SEQ_TAB  - '||SQLERRM );
      end;		
      --		 
   end if;	  
   --     
