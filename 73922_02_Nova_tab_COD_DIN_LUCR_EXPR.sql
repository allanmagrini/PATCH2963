  ...
  begin
   ...
   begin
      select count(1) 
       ...
       where upper(trim(s.sequence_name)) = 'CODDINLUCREXPL_SEQ'
         and upper(trim(s.table_name))    = 'COD_DIN_LUCR_EXPL';	  
   exception
      when others then
         vn_qtde := 0;
   end;
   ...
   if vn_qtde = 0 then
      ...
      exception
         when dup_val_on_index then
            null;	  
         when others then
            RAISE_APPLICATION_ERROR ( -20101, 'Erro ao inserir sequence CODDINLUCREXPL_SEQ na tabela SEQ_TAB  - '||SQLERRM );
      end;		
      --		 
   end if;	  
   --     
