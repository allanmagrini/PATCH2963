  ...
  begin
   ...
   begin
      select count(1) 
       ...
       where upper(trim(s.sequence_name)) = 'CALCBENEFLUCROEXPL_SEQ'
         and upper(trim(s.table_name))    = 'CALC_BENEF_LUCRO_EXPL';	  
   exception
      when others then
         vn_qtde := 0;
   end;
      ...
      begin
         insert into csf_own.seq_tab ( id
                             ...
                             values ( csf_own.seqtab_seq.nextval
                                    , 'CALCBENEFLUCROEXPL_SEQ'
                                    , 'CALC_BENEF_LUCRO_EXPL');
      exception
         when dup_val_on_index then
            null;	  
         when others then
            RAISE_APPLICATION_ERROR ( -20101, 'Erro ao inserir sequence CALCBENEFLUCROEXPL_SEQ na tabela SEQ_TAB  - '||SQLERRM );
      end;		
      --		 
   end if;	  
   --     
