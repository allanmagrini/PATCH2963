  ...
  begin
   ...
   begin
      select count(1) 
       ...
       where upper(trim(s.sequence_name)) = 'FORMULACODDINLUCEXP_SEQ'
         and upper(trim(s.table_name))    = 'FORMULA_COD_DIN_LUCEXP';	  
   exception
      when others then
         vn_qtde := 0;
   end;
      ...
      begin
         insert into csf_own.seq_tab ( id
                             ...
                             values ( csf_own.seqtab_seq.nextval
                                    , 'FORMULACODDINLUCEXP_SEQ'
                                    , 'FORMULA_COD_DIN_LUCEXP');
      exception
